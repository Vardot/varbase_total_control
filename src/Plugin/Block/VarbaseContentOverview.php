...
/**
 * @file
 * Contains \Drupal\varbase_total_control\Plugin\Block\VarbaseContentOverview.
 */
...
class VarbaseContentOverview extends BlockBase implements BlockPluginInterface {
  ...
  public function build() {
    ...
    foreach ($types as $type => $object) {
      ...
      if ((!array_key_exists($type, $config ['varbase_total_control_types_overview'])) || (isset($config ['varbase_total_control_types_overview']) && $config ['varbase_total_control_types_overview'] [$type]) == $type) {
        $type_count = db_query("SELECT count(*) FROM {node_field_data} WHERE type = :type and status = 1", array(
          ':type' => $type
          ))->fetchField();
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    $form ['varbase_total_control_types_overview'] = array(
      '#type' => 'checkboxes',
    ...
    $form ['varbase_total_control_comments_overview'] = array(
      '#type' => 'checkboxes',
    ...
    $form ['varbase_total_control_spam_overview'] = array(
      '#type' => 'radios',
